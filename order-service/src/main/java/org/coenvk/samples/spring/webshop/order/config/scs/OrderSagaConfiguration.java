...
import org.coenvk.samples.spring.webshop.order.service.OrderSagaOrchestrator;
...
public class OrderSagaConfiguration {
    private final OrderSagaOrchestrator _orderSagaOrchestrator;

    @Autowired
    public OrderSagaConfiguration(
    ...
    @Bean
    public Function<Flux<Message<?>>, Flux<Message<Object>>> sagaProcessor(
        ...
        return flux -> flux.flatMap(message -> {
            ...
            return Mono.fromSupplier(() -> {
                var payload = _orderSagaOrchestrator.on(event);
